...
class PatentAutomationApp:
    ...
    def retrieve_headers_from_csv(self):
        ...
        try:
            csv_path = os.path.join(os.path.dirname(__file__), 'config', 'patent_headers_default.csv')
            ...
            self.log_message(f"Retrieved {len(self.table_headers) - 1} basic headers and {len(self.ad_headers)} application detail headers from CSV", "STATUS")
        except Exception as e:
            self.log_message(f"Failed to retrieve headers from CSV: {str(e)}", "ERROR")
        ...
        try:
            ...
            for attempt in range(max_retries + 1):
                ...
                for app_number in current_applications:
                    ...
                    try:
                        self.log_message(f"Processing application {app_number} for {email} (Attempt {attempt + 1})", "STATUS")
                        ...
                        self.log_message(f"Successfully processed {app_number} for {email}", "SUCCESS")
                        
                    except Exception as e:
                        self.log_message(f"Attempt {attempt + 1}: Error processing application {app_number} for {email}: {str(e)}", "ERROR")
                ...
                if attempt < max_retries:
                    ...
                    if current_failed_applications:
                        ...
                        for app in current_failed_applications:
                            self.log_message(f"Failed to process application {app}", "ERROR")
            
            self.log_message(f"Processed {len(processed_numbers)} applications for {email}", "SUCCESS")
            if failed_applications:
                self.log_message(f"Failed to process {len(failed_applications)} applications for {email}", "WARNING")
                
        except Exception as e:
            self.log_message(f"Error processing applications for {email}: {str(e)}", "ERROR")
            raise

    def load_user_settings(self):
        """Load user settings from patent_settings_default.csv"""
        try:
            settings_path = os.path.join(os.path.dirname(__file__), 'config', 'patent_settings_default.csv')
            self.log_message(f"Loading user settings from: {settings_path}", "INFO")
            if os.path.exists(settings_path):
                with open(settings_path, 'r', newline='', encoding='utf-8') as f:
                    ...
                    for row in reader:
                        if len(row) != 2:
                            self.log_message(f"Skipping invalid row: {row}", "WARNING")
                            continue
                ...
                self.app_numbers_text.insert("1.0", settings.get("application_numbers", ""))
                self.extra_emails_text.insert("1.0", settings.get("extra_emails", ""))
                
                for header in self.table_headers:
                    try:
                        self.column_vars[header].set(int(settings.get(header, "1")))
                    except ValueError:
                        self.log_message(f"Skipping invalid entry for {header}", "WARNING")
                ...
                self.log_message(f"No user settings file found at: {settings_path}, using default settings", "WARNING")
                self.set_default_values()
        except Exception as e:
            self.log_message(f"Failed to load user settings: {str(e)}", "ERROR")
            self.set_default_values()
    ...
    def save_user_settings(self):
        ...
        try:
            settings_path = os.path.join(os.path.dirname(__file__), 'config', 'patent_settings_default.csv')
            self.log_message(f"Saving user settings to: {settings_path}", "INFO")
            with open(settings_path, 'w', newline='', encoding='utf-8') as f:
                ...
                writer.writerow(["application_numbers", self.app_numbers_text.get("1.0", tk.END).strip()])
                writer.writerow(["extra_emails", self.extra_emails_text.get("1.0", tk.END).strip()])
            ...
            self.log_message(f"User settings saved successfully to {settings_path}", "DEBUG")
        except Exception as e:
            self.log_message(f"Failed to save user settings: {str(e)}", "ERROR")
    ...
    def show_column_selection(self):
        ...
        column_window.transient(self.root)  # Set as transient to the main window
        ...
        ttk.Label(basic_frame, text="Note: 'Application Number' is always included").pack(anchor=tk.W, padx=5, pady=5)
        ...
        for header in sorted(self.table_headers):
            if header != "Application Number":  # Application Number is always included
                if header not in self.column_vars:
                    self.column_vars[header] = IntVar(value=0)  # Default to deselected
                ...
                Checkbutton(frame, text=header, variable=self.column_vars[header]).pack(side=tk.LEFT)
                
                operator_var = self.filter_settings[header]['operator_var']
                ttk.Combobox(frame, textvariable=operator_var, values=["--", "IS", "BLANK", "CONTAINS", "STARTS WITH", "ENDS WITH"], width=10).pack(side=tk.LEFT, padx=5)
        ...
        ttk.Button(btn_frame1, text="Select All Basic", command=self.select_all_basic_columns).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_frame1, text="Deselect All Basic", command=self.deselect_all_basic_columns).pack(side=tk.LEFT, padx=5)
        ...
        ttk.Label(ad_frame, text="Select Application Detail columns to include:").pack(anchor=tk.W, padx=5, pady=5)
        
        # Create a scrollable frame for AD columns
        ad_canvas = tk.Canvas(ad_frame)
        ad_scrollbar = ttk.Scrollbar(ad_frame, orient="vertical", command=ad_canvas.yview)
        ...
        ad_canvas.create_window((0, 0), window=ad_scrollable_frame, anchor="nw")
        ...
        for header in sorted(self.ad_headers):
            if header not in self.ad_column_vars:
                self.ad_column_vars[header] = IntVar(value=0)  # Default to deselected
            if header not in self.filter_settings:
                self.filter_settings[header] = {
                    'operator': "--",
                    'value': "",
            ...
            Checkbutton(frame, text=display_name, variable=self.ad_column_vars[header]).pack(side=tk.LEFT)
        ...
        ttk.Button(btn_frame2, text="Select All Details", command=self.select_all_ad_columns).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_frame2, text="Deselect All Details", command=self.deselect_all_ad_columns).pack(side=tk.LEFT, padx=5)
        
        # Add OK and Reset buttons at the bottom of the window
        btn_frame3 = ttk.Frame(column_window)
        btn_frame3.pack(fill=tk.X, pady=10)
        
        ttk.Button(btn_frame3, text="OK", command=lambda: self.save_and_close_column_window(column_window)).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_frame3, text="Reset", command=self.reset_filter_settings).pack(side=tk.LEFT, padx=5)


    def save_filter_settings(self):
        """Save filter settings to the patent_filters_default.csv file"""
        try:
            csv_path = os.path.join(os.path.dirname(__file__), 'config', 'patent_filters_default.csv')
            with open(csv_path, 'w', newline='', encoding='utf-8') as f:
                writer = csv.writer(f)
                for header, settings in self.filter_settings.items():
                    writer.writerow([header, settings['operator'], settings['value']])
            self.log_message("Filter settings saved successfully", "STATUS")
        except Exception as e:
            self.log_message(f"Failed to save filter settings: {str(e)}", "ERROR")  
    ...
    def reset_filter_settings(self):
        ...
        for header in self.table_headers:
            if header != "Application Number":  # Application Number is always included
                self.filter_settings[header] = {'operator': "--", 'value': "", 'operator_var': StringVar(value="--"), 'value_var': StringVar(value="")}
    ...
    def update_headers(self, basic_headers, ad_headers):
        ...
        for header in ad_headers:
            if header not in self.ad_headers:
                self.ad_headers.append(header)
                self.ad_column_vars[header] = IntVar(value=1)  # Default to selected
    ...
    def generate_filtered_data(self):
        ...
        try:
            # Define the path for the patent_full_list.csv file
            input_file = os.path.join(os.path.dirname(__file__), 'config', 'patent_full_list.csv')
            ...
            filtered_file = os.path.join(results_folder, f'filtered_patent_list_{timestamp}.csv')
            
            # Get headers from patent_headers_default.csv
            headers_path = os.path.join(os.path.dirname(__file__), 'config', 'patent_headers_default.csv')
            with open(headers_path, 'r', newline='', encoding='utf-8') as f:
                reader = csv.reader(f)
                headers = next(reader)
            
            # Filter out unselected columns
            selected_headers = [header for header in headers if self.column_vars.get(header, IntVar(value=0)).get() == 1]
            ...
            with open(filtered_file, 'w', newline='', encoding='utf-8') as f:
                ...
                for row in filtered_rows:
                    filtered_row = {header: row[header] for header in selected_headers if header in row}
                    writer.writerow(filtered_row)
            
            self.log_message(f"Filtered data saved to {filtered_file}", "SUCCESS")
            self.open_output_file(filtered_file)
        except Exception as e:
            self.log_message(f"Failed to generate filtered data: {str(e)}", "ERROR")
    ...
    def retrieve_filter_settings(self):
        ...
        try:
            ...
            if os.path.exists(csv_path):
                ...
                self.log_message("Filter settings loaded successfully", "STATUS")
            else:
                self.log_message("No filter settings file found, using default settings", "WARNING")
        except Exception as e:
            self.log_message(f"Failed to load filter settings: {str(e)}", "ERROR")
    ...
    def full_workflow(self):
        try:
            ...
            extra_emails_text = self.extra_emails_text.get("1.0", tk.END).strip()
            ...
            for email_index, current_email in enumerate(all_emails, 1):
                ...
                try:
                    ...
                    for retry in range(max_retries):
                        try:
                            ...
                            if retry == max_retries - 1:
                                raise
                            self.log_message(f"Retry {retry + 1} for navigation: {str(e)}", "WARNING")
                            time.sleep(2)
                    ...
                    self.log_message(f"Error processing email {current_email}: {str(e)}", "ERROR")
                    self.log_message(traceback.format_exc(), "ERROR")
                    continue
            ...
            self.running = False
            self.root.after(0, lambda: self.fetch_applications_btn.config(state=tk.NORMAL))
    ...
    def click_action_button_robust(self):
        ...
        self.log_message("Clicking action button with robust handling...", "STATUS")
        ...
        for selector in button_selectors:
            try:
                ...
                try:
                    ...
                    button.click()
                    self.log_message("Button clicked successfully with standard approach", "SUCCESS")
                    ...
                    self.log_message("Stale element detected, trying alternative approach", "WARNING")
                except Exception as e:
                    self.log_message(f"Standard click failed: {str(e)}", "WARNING")
                ...
                try:
                    ...
                    if "patent" in self.driver.current_url.lower():
                        self.log_message("Navigation successful with JavaScript click", "SUCCESS")
                        time.sleep(2)
                        return
                except Exception as e:
                    self.log_message(f"JavaScript click failed: {str(e)}", "WARNING")
                ...
                try:
                    ...
                    # Verify navigation success
                    if "patent" in self.driver.current_url.lower():
                        self.log_message("Navigation successful with action chains", "SUCCESS")
                        time.sleep(2)
                        return
                except Exception as e:
                    self.log_message(f"Action chains failed: {str(e)}", "WARNING")
                
            except Exception as e:
                self.log_message(f"Selector {selector} failed: {str(e)}", "WARNING")
        ...
        try:
            ...
            for button in buttons:
                try:
                    if button.is_displayed() and button.is_enabled():
                        ...
                        self.log_message(f"Found potential button: {button_text}", "STATUS")
                        ...
                        # Verify navigation success
                        if "patent" in self.driver.current_url.lower():
                            self.log_message("Navigation successful with generic button", "SUCCESS")
                            time.sleep(2)
                            return
                except StaleElementReferenceException:
                    continue
            ...
            self.log_message(f"Generic button approach failed: {str(e)}", "WARNING")
        
        # Final attempt: Direct navigation
        try:
            self.log_message("Trying direct navigation...", "STATUS")
            self.driver.get("https://sp-wildfly-cipo-itm-patents-sp-prod.apps.ocp.prod.ised-isde.canada.ca/backoffice/patent/application")
            time.sleep(5)
    ...
    def extract_table_data(self):
        ...
        try:
            ...
            if table_headers and len(table_headers) > 0:
                ...
                self.log_message("No table headers found, using default headers", "WARNING")
    ...
    def get_application_details(self, app_number):
        ...
        try:
            ...
            self.log_message(f"Error getting application details for {app_number}: {str(e)}", "ERROR")
            raise
    ...
    def update_csv_with_specific_row(self, csv_path, app_number, app_details_headers_col1, app_details_headers_col3, app_details_data_col1, app_details_data_col3):
        ...
        try:
            ...
            base_headers = ["Application Number"]  # Always include Application Number
            ...
            if new_ad_headers:
                self.log_message(f"Found {len(new_ad_headers)} new application detail headers", "STATUS")
            
            # Filter application detail headers based on selection
            filtered_headers_col1 = [h for h in unique_headers_col1 if h in selected_ad_columns]
            filtered_headers_col3 = [h for h in unique_headers_col3 if h in selected_ad_columns]
            
            # Create filtered data based on selected headers
            filtered_data_col1 = [v for h, v in zip(unique_headers_col1, app_details_data_col1) if h in selected_ad_columns]
            filtered_data_col3 = [v for h, v in zip(unique_headers_col3, app_details_data_col3) if h in selected_ad_columns]
    ...
    def process_application_details(self, app_number, csv_file, email):
        ...
        try:
            ...
            if details_data and 'col1_headers' in details_data:
                ...
                self.log_message(f"Failed to extract details for {app_number}", "ERROR")
                raise Exception("Details extraction failed")
                
        except TimeoutException as te:
            self.log_message(f"Timeout processing {app_number}: {str(te)}", "ERROR")
            raise
        except Exception as e:
